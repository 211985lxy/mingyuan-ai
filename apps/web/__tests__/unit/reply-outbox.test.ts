...
import { OUTBOX_SEND_TASK_KIND, enqueueReply, acknowledgeOutboxReply, claimOutboxReplies, MAX_OUTBOX_ATTEMPTS } from "@/features/topics/services/reply-outbox"
...
describe("reply-outbox", () => {
  ...
  describe("enqueueReply", () => {
    it("creates an outbox record with correct fields", async () => {
      mocks.channelReplyOutbox.create.mockResolvedValue({ id: "outbox-1", status: "pending" })
      await enqueueReply({
      ...
      expect(mocks.channelReplyOutbox.create).toHaveBeenCalledWith({
    ...
    it("enqueues a background task for feishu platform", async () => {
      mocks.channelReplyOutbox.create.mockResolvedValue({ id: "outbox-1", status: "pending" })
      await enqueueReply({
        inspirationId: "insp-1",
        replyType: "final",
      ...
      expect(mockedEnqueue).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          idempotencyKey: "outbox-send:outbox-1",
        }),
      )
    })
    ...
    it("does not enqueue a background task for external platforms", async () => {
      mocks.channelReplyOutbox.create.mockResolvedValue({ id: "outbox-2", status: "pending" })
      await enqueueReply({
        inspirationId: "insp-1",
        replyType: "final",
        platform: "wecom",
    ...
    it("supports error reply type", async () => {
      mocks.channelReplyOutbox.create.mockResolvedValue({ id: "outbox-err", status: "pending" })
      await enqueueReply({
        inspirationId: "insp-1",
        replyType: "error",
        platform: "feishu",
      ...
      expect(mocks.channelReplyOutbox.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ replyType: "error" }),
        }),
      )
    })
  })
