...
describe("assets upload proxy put route（同源兜底上传）", () => {
  ...
  it("缺 file 拒绝（400）", async () => {
    const response = await POST(putRequest("up-1", null), { params: Promise.resolve({ uploadId: "up-1" }) })
  ...
  it("预约校验通过 → 服务端写入 OSS 并返回 200", async () => {
    ...
    const response = await POST(putRequest("up-1", file), { params: Promise.resolve({ uploadId: "up-1" }) })
  ...
  it("预约不存在返回路由给的 404", async () => {
    ...
    const file = new File(["12345"], "a.png", { type: "image/png" })
    const response = await POST(putRequest("missing", file), { params: Promise.resolve({ uploadId: "missing" }) })
