...
const { findFirst, authenticateRequest, authErrorResponse, resolveBoundProject, store } = vi.hoisted(() => ({
  ...
  store: {
    ...
    findByExternalLeadId: vi.fn(async (_externalLeadId: string): Promise<Record<string, unknown> | null> => null),
    findByExternalDealId: vi.fn(async (_dealId: string): Promise<Record<string, unknown> | null> => null),
