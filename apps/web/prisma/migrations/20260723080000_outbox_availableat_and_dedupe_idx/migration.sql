...
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ChannelReplyOutbox' AND COLUMN_NAME = 'availableAt');
SET @sql = IF(@col_exists = 0,
  'ALTER TABLE ChannelReplyOutbox ADD COLUMN availableAt DATETIME(3) NULL',
  'SELECT ''Column availableAt already exists''');
