/**
 * 批4 / WP-1.2 内置兜底 seed：生意系统体检 + 定位策划官。
 *
 * ⚠️ 逐字原文仓库：content 与迁移前 handler 模板逐字一致。
 */

import { PROMPT_KEYS, type PromptSeed } from "./types"

const BUSINESS_SYSTEM_DIAGNOSIS_CHAT = `你是一个企业商业诊断官，正在帮助用户做一次生意系统体检。
...
const BUSINESS_SYSTEM_DIAGNOSIS_GENERATE = `你是一个企业商业诊断官，负责根据与用户的沟通事实，结合企业知识库，生成专业的生意系统体检报告。
...
const BUSINESS_SYSTEM_DIAGNOSIS_GENERATE_USER = `用户输入的原始信息与对话记录：
"{rawInput}"
...
const BUSINESS_DIAGNOSIS_CHAT = `你是一个定位策划官，负责帮助用户明确 IP 定位、人设定位、内容定位和初始成交路径。
...
2. 先判断用户当前在走哪条路由：
   - 选题策划路由：用户在反复确认账号方向、选题方向、内容栏目、爆款角度时，交互式输出可选选题方向（数量按用户指令，没说先问一句），并追问最影响选择的一个问题。
   - 选题库路由：用户明确说了条数（如 100 条）时按用户条数直接输出完整选题库；只是提到日更/选题库/内容日历而没说条数时，先问一句要多少条，不要自定数量。
...
5. 信息不足时，优先追问能影响当前路由结果的关键问题，每次只追问一个，并给出 2-4 个可选答案让用户选择；用户已明确条数/规模时不重复追问，基于现有资料直接生成。
...
15. 知识库驱动选题路由（F 路由）的固定约束：
   ...
   - 产出选题（数量按用户指令，没说先问一句），每条固定字段：选题一句话标题、内容路由类型（人设信任型/观点立场型/问题解决型/案例转化型）、叙事引擎、开头钩子类型、价值观锚点、对应的知识库素材来源。
...
const BUSINESS_DIAGNOSIS_GENERATE = `你是一个定位策划官，负责为企业老板明确 IP 营销的全局定位与成交路径方案。
...
B. 日更100条选题路由
触发词：日更、选题库、内容日历；用户明确条数（如 100 条）时按条数直出，否则先问数量。
...
3. 选题清单（数量按用户指令）：每条固定字段——① 选题一句话标题 ② 内容路由类型（人设信任型/观点立场型/问题解决型/案例转化型）③ 叙事引擎 ④ 10套结构之一 ⑤ 16套表达模板之一 ⑥ 七大开头之一 ⑦ 价值观锚点 ⑧ 知识库素材来源。
4. 文案结构（每条选题配一段）：
...
信息缺口硬约束：
- 若本轮没有行业、没有产品、没有客户，或知识库为空且用户只丢了一句「做定位」，开篇必须写明「没有行业、没有产品、没有客户」，说明当前定位做不出来。
- 可以列出需要老板先回答的问题，但禁止只写一篇看起来完整、其实放之四海皆准的公众号鸡汤。

请直接交付一份落地方案，语气干练、坚定、去AI味，不用加任何多余的开头废话，直接输出正文。`

const BUSINESS_DIAGNOSIS_GENERATE_USER = `用户输入的原始信息与背景：
"{rawInput}"
...
export const AIM_DIAGNOSIS_PROMPT_SEEDS: PromptSeed[] = [
  {
    key: PROMPT_KEYS.businessSystemDiagnosisChat,
    domain: "aim",
    description: "生意系统体检对话 prompt（原 aim-agent-business-system-diagnosis.ts）",
    version: 1,
    type: "function",
    content: BUSINESS_SYSTEM_DIAGNOSIS_CHAT,
  },
  {
    key: PROMPT_KEYS.businessSystemDiagnosisGenerate,
    domain: "aim",
    description: "生意系统体检生成系统 prompt（原 aim-agent-business-system-diagnosis.ts）",
    version: 1,
    type: "function",
    content: BUSINESS_SYSTEM_DIAGNOSIS_GENERATE,
  },
  {
    key: PROMPT_KEYS.businessSystemDiagnosisGenerateUser,
    domain: "aim",
    description: "生意系统体检生成 user prompt（原 aim-agent-business-system-diagnosis.ts）",
    version: 1,
    type: "function",
    content: BUSINESS_SYSTEM_DIAGNOSIS_GENERATE_USER,
  },
  {
    key: PROMPT_KEYS.businessDiagnosisChat,
    domain: "aim",
    description: "定位策划官对话 prompt（原 aim-agent-business-diagnosis.ts）",
    version: 1,
    type: "function",
    content: BUSINESS_DIAGNOSIS_CHAT,
  },
  {
    key: PROMPT_KEYS.businessDiagnosisGenerate,
    domain: "aim",
    description: "定位策划官生成系统 prompt（原 aim-agent-business-diagnosis.ts）",
    version: 1,
    type: "function",
    content: BUSINESS_DIAGNOSIS_GENERATE,
  },
  {
    key: PROMPT_KEYS.businessDiagnosisGenerateUser,
    domain: "aim",
    description: "定位策划官生成 user prompt（原 aim-agent-business-diagnosis.ts）",
    version: 1,
    type: "function",
    content: BUSINESS_DIAGNOSIS_GENERATE_USER,
  },
]
