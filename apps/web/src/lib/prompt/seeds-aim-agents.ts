...
2. 润色：保留作者立场、关键事实和真实数据，明显去 AI 味，纠正错别字和病句；不要擅自改主题或扩写成全新长文。只交一版可直接替换的成稿，禁止输出「润色候选」「3选1」「任选其一」「备选」或改动说明。
...
- 润色：保真、去 AI 味、不改立场与关键数据；默认保留篇幅，除非用户明确要求精简。只交一版成稿，禁止候选清单或让用户挑选。
