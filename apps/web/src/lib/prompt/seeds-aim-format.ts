/**
 * 批5 / WP-1.2 下一刀：格式指令、小红书导演、脚本 system、闭集事实。
 *
 * ⚠️ 逐字原文仓库：content 与迁移前 FORMAT_INSTRUCTIONS / generation.ts / scoring.ts 逐字一致。
 */

import { PROMPT_KEYS, type PromptSeed } from "./types"

export const AIM_FORMAT_PROMPT_SEEDS: PromptSeed[] = [
  {
    key: PROMPT_KEYS.formatVideoScript,
    domain: "aim",
    description: "口播/短视频格式指令（原 FORMAT_INSTRUCTIONS.video_script）",
    version: 1,
    type: "system",
  ...
  {
    key: PROMPT_KEYS.formatWechatArticle,
    domain: "aim",
    description: "公众号文章格式指令（原 FORMAT_INSTRUCTIONS.wechat_article）",
    version: 1,
    type: "system",
  ...
  {
    key: PROMPT_KEYS.formatMomentsPost,
    domain: "aim",
    description: "朋友圈格式指令（原 FORMAT_INSTRUCTIONS.moments_post）",
    version: 1,
    type: "system",
  ...
  {
    key: PROMPT_KEYS.formatCommunityMessage,
    domain: "aim",
    description: "社群运营格式指令（原 FORMAT_INSTRUCTIONS.community_message）",
    version: 1,
    type: "system",
  ...
  {
    key: PROMPT_KEYS.formatRawCopy,
    domain: "aim",
    description: "原始文案格式指令（原 FORMAT_INSTRUCTIONS.raw_copy）",
    version: 1,
    type: "system",
  ...
  {
    key: PROMPT_KEYS.formatShootingBrief,
    domain: "aim",
    description: "拍摄交接单格式指令（原 FORMAT_INSTRUCTIONS.shooting_brief）",
    version: 1,
    type: "system",
  ...
  {
    key: PROMPT_KEYS.formatXiaohongshuPost,
    domain: "aim",
    description: "小红书图文视觉导演指令（原 buildXhsVisualDirectorInstruction）",
    version: 1,
    type: "system",
  ...
  {
    key: PROMPT_KEYS.scriptGenerationScoringSystem,
    domain: "aim",
    description: "脚本评分专家 system（原 scoring.ts）",
    version: 1,
    type: "system",
    content: "你是一位严格的短视频文案质量审核专家。只输出纯 JSON，不添加任何说明。",
  },
  {
    key: PROMPT_KEYS.contentProducerClosedSetFacts,
    domain: "aim",
    description: "闭集事实快路径附加指令（原 aim-agent-content-producer.ts）",
    version: 1,
    type: "system",
    content: "这是闭集事实任务：只使用用户原始输入里的事实、客户信息和数字，不调用或复述其他背景事实。客户案例段只能逐字引用用户原文里的事实锚点；禁止补充人员、流程、渠道、做法、原因、其他结果或因果解释，禁止计算、换算或概括降幅、比例等衍生数字，禁止用“他们”“该公司”“这家公司”引出任何新信息。事实锚点之前必须完整展开目标客户、用户明确写出的痛点、问题机制和不含新增事实或数字的可执行判断，篇幅必须匹配用户指定时长，不能缩成几句话。结尾只执行用户指定的行动引导，不增加免费、保证、限时或交付承诺。直接输出完整成稿，不解释、不分析、不增加案例细节。",
  },
]
