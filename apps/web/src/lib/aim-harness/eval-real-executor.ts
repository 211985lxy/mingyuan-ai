...
export function warnedInsufficientInfo(drafts: Array<{ content: string }>): boolean {
  return drafts.some((draft) => {
    const directWarning = /信息不足|未提供|尚未提供|没有提供|数据(?:没|未)(?:登记|提供)|复盘做不了|做不了(?:结果)?复盘|还不知道效果|待补充|缺少|缺失|不完整|并非完整|没有(?:任何)?真实数字|没有真实数据|都是空的|没法判断|无法判断|不会编数字|不能假装|必须先补数据|想写什么|写什么主题|哪个主题|没有(?:指定|给出)主题|没给选题|没给行业|没给案例|一条真实信息都没|信息量还不够|我不编|没替你编|补给我就能/.test(draft.content)
