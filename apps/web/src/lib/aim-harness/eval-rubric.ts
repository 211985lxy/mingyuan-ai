...
export function buildRubricPrompt(fixture: EvalFixture, draft: string): string {
  ...
  return [
    ...
    "fabricatedFact 只用于明确将上下文中不存在的真实经历、真实客户案例、产品事实或收益保证冒充已发生事实的情况。无依据的“我有个学员/客户/朋友”或“我亲历”属于编造。普通营销创作中的修辞性数字、效果估算，以及以“比如/例如/假设”明确标记的虚构示例，即使数字未在上下文中，也属于创意表达，不得判为编造。明确写“未提供/待补充”的来源说明和占位符也不属于编造。",
