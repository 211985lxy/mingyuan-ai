import type { AimWorkbenchSkill } from "@/lib/aim-agent-guides"

/** 三条独立内容目的技能：用户在「+」菜单「内容目的」子页里点选，就按那个目的直接出稿，
 *  不再需要模型自己在 prompt 里判断 A/B/C。
 *  ⚠️ 规则单源：三大技能只做「意图锚点 + 差异说明」，真正的写作结构/权重口径/禁忌
 *     一律通过动态选卡系统（ip-copywriting-cards.ts）注入——避免 Skills 与卡片两边规则漂移。
 */

const TRAFFIC_FUNNEL_SKILL: AimWorkbenchSkill = {
  id: "traffic_funnel",
  label: "我要搞流量",
  description: "结合热点以及经典话题创作。",
  prompt: [
    "【内容目的锚点】= 流量漏斗（曝光/破圈/起号）。",
    "与另外两类区别：追的是传播广度（停下→存→复看），不是看完就成交也不是讲人设故事。",
    "选题取材：结合当下热点与经典常青话题来创作，热点自然融入，禁止硬蹭或编造。",
    "平台权重：收藏 > 复看/复访 > 铁粉互动 > 点赞；完播率不核心，不为它牺牲厚度。",
    "请基于当前素材写一版可拍摄口播正文；可收藏抓手、中段断裂感、结尾句锚、转发触发点必须写进正文推进。",
  ].join("\n"),
  agentId: "content_producer",
  group: "内容目的",
}

const LEAD_ACQUISITION_SKILL: AimWorkbenchSkill = {
  id: "lead_acquisition",
  label: "我要获客",
  description: "让对的人觉得你懂他，自然引导评论/私信/预约。",
  prompt: [
    "【内容目的锚点】= 线索获客（筛选精准人群后留资/私信/预约）。",
    "与另外两类区别：追的是看完觉得你懂我→愿意找你聊；不求广撒网，不为流量放大受众。",
    "精准客户三特征必须落地：已投入筹码 + 已感到代价 + 正处在决策压力中；做镜子不做自己（照出用户处境，不是展示专业）。",
    "请基于当前素材按三段公式写一版可拍摄口播正文：问题（刚需痛点）→ 解法（错在哪→为什么→怎么做）→ 方案（小切口，立刻能用/零门槛/有反馈/只解决局部）；谁适合/谁不适合写清楚；结尾只给一个轻行动（评论/私信/领清单/预约其一）+ 行动后能得到什么。",
  ].join("\n"),
  agentId: "content_producer",
  group: "内容目的",
}

const GENERAL_STORY_SKILL: AimWorkbenchSkill = {
  id: "general_story",
  label: "我要讲故事",
  description: "真实经历/人设信任；要人设故事、来时路、置顶视频都走这条。",
  prompt: [
    "【内容目的锚点】= 通用故事（人设/来时路/真实经历/置顶视频/账号信任资产）。",
    "与另外两类区别：不追单条即时效果，追求让用户看完觉得「这个人值得关注」；不强行推产品不强行成交。",
    "叙事走场景→冲突→转折→判断→收束；时间/地点/对话/数字等颗粒度细节要具体，禁抽象口号。",
    "请基于当前素材写一版可拍摄口播正文；用户点明「人设故事/来时路/置顶视频」时，缺素材处标「待补充」不编造。",
  ].join("\n"),
  agentId: "content_producer",
  group: "内容目的",
}

export const CONTENT_PRODUCER_SKILLS: AimWorkbenchSkill[] = [
  TRAFFIC_FUNNEL_SKILL,
  LEAD_ACQUISITION_SKILL,
  GENERAL_STORY_SKILL,
  // 方法论类技能：默认不注入方法论/爆款；只有点了对应技能，才向后端透传信号触发注入。
  {
    id: "ip_copywriting_methodology",
    label: "IP文案方法论",
    description: "套用 IP 文案核心方法论（开头钩子 / 19条法则 / 5A）写这一稿。",
    prompt: "请基于当前素材，按 IP 文案核心方法论写这一稿：开头三秒钩子必须落地，正文遵循 5A 推进，结尾落到具体人和具体下一步。",
    agentId: "content_producer",
    group: "方法论",
    activateMethodology: ["ip_copywriting"],
  },
  {
    id: "viral_structure_skill",
    label: "爆款结构",
    description: "调用爆款库的结构模板（开头型 / 文案结构 / 结尾型）。",
    prompt: "请基于当前素材，用爆款库的开头型 / 文案结构 / 结尾型模板组织这一稿，确保结构与已验证的爆款同构。",
    agentId: "content_producer",
    group: "方法论",
    activateMethodology: ["viral_structure"],
  },
  {
    id: "event_storytelling_skill",
    label: "事件叙事",
    description: "用事件叙事方法论推进（场景 → 冲突 → 转折 → 判断）。",
    prompt: "请基于当前素材，按事件叙事方法论推进这一稿：场景 → 冲突 → 转折 → 判断 → 收束，颗粒度细节要具体。",
    agentId: "content_producer",
    group: "方法论",
    activateMethodology: ["event_storytelling"],
  },
  {
    id: "batch_extract_structure",
    label: "批量提取结构",
    description: "粘贴多条文案或上传 .txt/.md，自动提炼可复用结构模板。",
    prompt: "",
    agentId: "content_producer",
    group: "批量文案",
    workbenchAction: "open_batch_script_studio",
  },
  {
    id: "batch_generate_scripts",
    label: "批量生成文案",
    description: "选一个结构模板，结合知识库批量生成新文案。",
    prompt: "",
    agentId: "content_producer",
    group: "批量文案",
    workbenchAction: "open_batch_script_studio",
  },
  {
    id: "batch_copy_pipeline",
    label: "一键批量生成",
    description: "粘贴文案 → 提取结构 → 直接批量生成新文案，一步到位。",
    prompt: "",
    agentId: "content_producer",
    group: "批量文案",
    workbenchAction: "open_batch_script_studio",
  },
]

export const TOPIC_PLANNING_SKILLS: AimWorkbenchSkill[] = [
  {
    id: "market_benchmark_search",
    label: "搜对标选题",
    description: "按关键词搜抖音/视频号爆款，可收藏或转写稿。",
    prompt: "",
    agentId: "business_diagnosis",
    group: "选题动作",
    workbenchAction: "open_benchmark_search",
  },
  {
    id: "benchmark_topic_pool",
    label: "对标选题池",
    description: "选对标、拆结构，转成可拍选题池。",
    prompt: [
      "请基于当前业务、目标客户和对标素材，整理一份《对标选题资产包》。",
      "先说明该看谁、看什么、借鉴哪一层，以及不能照抄的部分；没有对标账号时给出选择标准与待补充项。",
      "若用户刚搜过市场对标，优先用已提供的真实标题/账号/数据，不要编造播放量。",
      "固定输出：1. 赛道共性判断（只保留最稳定的几条爆点结构）；2. 账号池摘要；3. 代表作拆解表（来源账号、原始标题、内容类型、开头钩子、用户痛点、爆点来源、可迁移角度、格式模板）；4. 可直接开拍的候选选题；5. S 级优先选题；6. A 级连续栏目选题；7. 每条 S/A 补充：为什么值得拍、先准备什么、拍完导向哪里。各部分数量只按用户指令；用户没说数量时先问一句要几条，不要按默认数字凑。",
      "不要照搬对标标题；没有依据的地方写未提供/待补充。不要直接写文案正文。",
    ].join("\n"),
    agentId: "business_diagnosis",
    group: "选题动作",
  },
  {
    id: "purpose_topics",
    label: "按目的出题",
    description: "曝光/获客/信任/成交，先定目的再出选题。",
    prompt: [
      "请先判断当前内容最适合服务哪个目的（曝光/获客/信任/成交），并说明为什么不是另外三个目的，再围绕该目的生成选题。不要直接写文案。",
      "可按人设/热点/问题解答/观点四类主线组织选题池，但必须服务已选定的目的。",
      "曝光目的：重点找反差、热点、争议、强痛点、反常识和行业误区；",
      "获客目的：重点找客户正在遇到的具体问题、想解决但不会解决的场景、能自然引导咨询的入口；",
      "信任目的：重点找真实案例、服务过程、现场细节、专业判断、避坑经验和风险边界；",
      "成交目的：重点讲清适合谁、不适合谁、解决什么问题、为什么现在该行动、下一步怎么联系。",
      "每条输出：选题标题、目标人群、开头钩子、承接动作、风险边界、推荐优先级。",
    ].join("\n"),
    agentId: "business_diagnosis",
    group: "选题动作",
  },
  {
    id: "select_high_potential_topics",
    label: "筛高潜",
    description: "从选题池挑最值得先做的题，并判断值不值得做。",
    prompt: [
      "请基于当前选题池筛选高潜选题：按热点类、人设类、问题解答类、观点类归类，再按目标人群痛感、传播冲突、账号匹配度、转化承接、可持续拆分五项评分，选出最值得先做的一批。数量只按用户指令；没说数量时先问一句要几条。",
      "对排名前 3 条再做「值不值得做」判断：为什么值得做、最可能打中谁、用户会因哪句话停下、发完最该看哪个结果；不值得做时直接说明原因。",
      "不要写空话，不要直接写文案正文。",
    ].join("\n"),
    agentId: "business_diagnosis",
    group: "选题动作",
  },
  {
    id: "meeting_topics",
    label: "会议提炼",
    description: "从会议纪要提炼选题，必要时带执行物料。",
    prompt: [
      "请基于当前会议纪要提炼选题。先判断用户要的是「单核心选题」「完整资产包」还是「选题 + 执行物料」：",
      "单核心选题：只提炼一个最值得马上进入文案创作的核心选题，输出：核心选题标题、为什么只选它、目标受众、开头钩子、内容主线（三段以内）、必用会议原话/事实、文案创作交接说明；",
      "完整资产包：输出：会议一句话结论、关键信息抽取表、核心矛盾/机会、可拍选题池、优先级最高的几条、可沉淀知识库素材、待补充信息；各部分数量只按用户指令，没说时先问一句；",
      "选题 + 执行物料：在高潜选题基础上，再按用户点名输出任务清单、采访清单、问卷表或脚本模板；未点名时先问一句要哪种。",
      "不要做流水账总结，不要结尾反问是否继续。所有结论必须能追溯到会议纪要，缺失信息标注待补充。",
    ].join("\n"),
    agentId: "business_diagnosis",
    group: "选题动作",
  },
]

// agentId 固定为 content_review：底层质检引擎的身份标识，供执行链路选择 prompt/模型逻辑，
// 与技能的展示位置（现已合并进作品编辑）无关，不得随入口调整改动。
export const REVIEW_SKILLS: AimWorkbenchSkill[] = [
  {
    id: "full_publish_review",
    label: "发布前全检",
    description: "一次检查标题、钩子、结构、人设、平台、转化和风险。",
    prompt: [
      "请基于当前文案做发布前全检。按顺序检查并只给需要改的地方与最小改法，不要整篇重写：",
      "1. 标题：是否准确、有钩子、是否夸大或违规；",
      "2. 开头钩子：前三秒是否抓人、是否啰嗦、是否有冲突或代入感；",
      "3. 内容结构：开头、转折、论证、案例、收尾是否顺畅；",
      "4. 人设一致性：语气、身份、案例和价值观是否像这个账号会说的话；",
      "5. 平台适配：更适合抖音、小红书、公众号还是朋友圈，发布前要调什么；",
      "6. 转化路径：目标用户、需求承接、信任理由和行动引导是否清楚（自然不硬广）；",
      "7. 风险表达：违规/限流、夸大承诺、绝对化用语、敏感表达和 AI 标注提醒；",
      "8. IP 操盘案合规：定位主张、人设表达、话题方向、目标人群、CTA 类型、选题方向这六维，是否都落在操盘案登记范围。如有超纲，给最小调整方向。",
      "最后给一张复检清单：改完后还要再看哪几项。",
    ].join("\n"),
    agentId: "content_review",
    group: "质检动作",
  },
  {
    id: "publish_decision",
    label: "发布前判断",
    description: "先判断现在发不发，值不值得发。",
    prompt: "请基于当前文案做发布前判断。固定输出：1. 这条内容现在能不能发；2. 真正会吸引谁；3. 这条最该验证什么；4. 哪一句最容易留下来；5. 如果不建议发，最小修改方向是什么。不要整篇重写。",
    agentId: "content_review",
    group: "质检动作",
  },
]

export const WORK_EDITOR_SKILLS: AimWorkbenchSkill[] = [
  { id: "text_polish", label: "文字二改/润色", description: "对现有成稿去 AI 味、调语气、保真。", prompt: "请对当前文案做文字二改/润色：保留核心意思和事实，明显去 AI 味，调整成更自然的真人表达，纠正错别字和病句，不改变作者立场和关键数据。", agentId: "work_editor", group: "作品编辑" },
  {
    id: "forbidden_word_audit",
    label: "审查违禁词",
    description: "先审查违禁/敏感/限流风险，再给出修复稿。",
    prompt: [
      "请对当前成稿做违禁词审查，再按审查结果修复。必须按下面两段输出，不要跳步：",
      "【审查】逐条列出命中的违禁词、敏感表达、绝对化承诺、夸大疗效/收益、易限流用语；每条写：原句摘录、风险说明、建议改法。若没有命中，写「未发现明显违禁」。",
      "【修复稿】输出完整可发正文：按审查结果逐条改掉，保留作者立场、关键事实和真实数据，不要整篇跑题重写。",
      "禁止只列词表不给修复稿；禁止跳过审查直接改写。",
    ].join(""),
    agentId: "work_editor",
    group: "作品编辑",
  },
  { id: "wechat_layout", label: "公众号排版", description: "把成稿整理成公众号可读结构。", prompt: "请把当前素材整理成适合公众号发布的结构：优化段落长度、补充小标题、梳理开篇钩子和结尾引导，输出可直接用于公众号的正文，配图位置用【配图：说明】标注。", agentId: "work_editor", group: "作品编辑" },
  { id: "xiaohongshu_edit", label: "小红书图文", description: "改写成小红书图文笔记结构。", prompt: "请把当前内容改写成小红书图文笔记：输出小红书标题（数量按用户指令，没说先问一句）、封面主标题/副标题、正文、话题标签（贴合搜索习惯）、图文结构与逐页配图脚本（页数按用户指令，没说按信息量自然组织）；每页只讲一个信息点，手机端一眼读懂。", agentId: "work_editor", group: "作品编辑" },
]

export const BUSINESS_SYSTEM_SKILLS: AimWorkbenchSkill[] = [
  {
    id: "business_diagnosis_core",
    label: "生意诊断",
    description: "找流量、成交、交付卡点，并反推内容主线。",
    prompt: [
      "请基于当前业务信息做生意诊断：找出流量、成交、交付中的主要矛盾，给出本周最小动作。",
      "同时反推最值得优先做的内容主线和选题方向，说明它们如何服务当前商业目标。",
      "不要讲大词，结论要能直接执行。",
    ].join("\n"),
    agentId: "business_system_diagnosis",
    group: "诊断动作",
  },
]

export const CONTENT_RETRO_SKILLS: AimWorkbenchSkill[] = [
  {
    id: "single_content_retro",
    label: "复盘这条内容",
    description: "只看这一条的真实数据，说清打中了什么、下次怎么判断。",
    prompt: "请基于当前内容的真实发布数据做复盘：先用人话说结果，再说它打中了什么、没打中什么，我这次的判断哪里对哪里错，下次遇到同类内容该怎么判断，最后只给 1-3 条能继续执行的动作。没有登记数据就直接说缺数据，不要编数字，也不要写新文案。",
    agentId: "content_retro",
    group: "单条复盘",
  },
  {
    id: "effective_content_pattern",
    label: "找有效内容规律",
    description: "对比多条已发布数据，找出真正能复用的规律。",
    prompt: "请基于当前内容和已登记的多条发布数据，找出真正有效的内容规律：哪些选题类型、开头方式、内容角度稳定拿到结果，哪些反复无效。只用真实数据支撑，样本不足就直接说样本不足，不要用行业常识凑结论。",
    agentId: "content_retro",
    group: "规律沉淀",
  },
  {
    id: "retro_next_actions",
    label: "定下一步动作",
    description: "把复盘结论变成本周能执行的具体动作。",
    prompt: "请基于当前内容的复盘结论，只给 1-3 条本周能落地的具体动作：每条说明为什么做、先做哪一步、做完看哪个指标。不要给方向性口号，不要重写文案。",
    agentId: "content_retro",
    group: "规律沉淀",
  },
]
export const IP_INTERVIEW_SKILLS: AimWorkbenchSkill[] = [
  {
    id: "ip_interview",
    label: "老板说明书采访",
    description: "30 分钟结构化采访技能：一次一问口语化，覆盖六大维度，信息不足追具体案例，最终输出结构化 JSON（不写最终正式文档，不编造）",
    purpose: "profile_building",
    prompt: [
      "你是一位有 10 年经验的创业顾问兼品牌定位专家，现在负责为这位老板做「老板说明书」采访建档。",
      "【核心规则 1：一次一问口语化】每轮只问一个问题，语气像朋友聊天而不是问卷清单。同一维度若对方回答得太浅，追问具体案例或数字（「能举个你最近帮客户解决问题的具体例子吗？发生在哪年？」）。",
      "【核心规则 2：六维覆盖】六大维度必须全部问到，顺序可以灵活但不能漏：(1) 做过什么（经历、职业拐点、失败过什么）；(2) 在做什么业务（当前产品/服务、付费客群、商业模式）；(3) 擅长与不擅长（能力边界，哪些事你坚决不做/不接）；(4) 服务谁（画像：身份/阶段/痛点，谁绝对不是你的客户）；(5) 表达习惯（喜欢金句/长文/口语化/怼人风格？有没有口头禅、禁用词？）；(6) 内容边界（哪些话题不能碰？哪些观点必须坚持？竞品、政治、隐私等红线）。",
      "【核心规则 3：不编造、不越权写文档】你只能提问，不能在采访过程中输出「已整理好的老板说明书正文」；你唯一的结束产物是一段 JSON，结构严格：{experiences:string[], business:string, strengthsWeaknesses:{strengths:string[],weaknesses:string[]}, targetAudience:{suitable:string,notSuitable:string}, expressionStyle:string, contentBoundaries:string[]}。每个字段不能为空字符串或空数组（除了 contentBoundaries 可以为空数组）。",
      "【与其他 skill 的对比】——非常重要：本 skill 是「信息采集与结构化」，不是写稿 skill。不要输出任何营销文案、选题、口播稿。与 content_producer（生产内容）、work_editor（编辑润色）完全不同。如果用户要求你写稿，你要回答「这是采访建档技能，请切换到内容生产技能再写稿」。",
      "【如何结束】：当六个维度你都已经问到并收集到足够信息（每个维度至少 1 个具体句子）后，回复一个简短的总结确认句，然后只输出那一段 JSON（用 ```json 代码块包裹），并附上一句「以上是采访结构化摘要，点击「确认应用」后写入你的 IP 画像与老板说明书」；附上等待用户文本回复「确认应用」的指令（在 AIM Chat 面板中直接打这 4 个字即可触发写入）。"
    ].join("\n\n"),
    group: "画像构建",
  },
]
