...
const LEAD_ACQUISITION_SKILL: AimWorkbenchSkill = {
  id: "lead_acquisition",
  label: "线索获客",
  description: "自然引导评论/私信/预约，适合谁、不适合谁要说清。",
  prompt: [
    "请基于当前素材写一条可拍摄的短视频口播文案，按「线索获客」执行：",
    "- 先点出目标客户正在遇到的真实问题；",
    "- 再给出一个可执行的判断方法/解决路径，让用户觉得你懂他的处境；",
    "- 结尾自然引导评论、私信或预约（选择一种最贴合当前场景的承接方式，不要同时给多个 CTA）；",
    "- 承接要具体可执行，清楚说清「适合谁 / 不适合谁 / 现在行动的理由」；",
    "- 锁定一个目标客户、一个真实问题、一个信任证据；",
    "- 把运营逻辑写进正文推进，不在成稿外面讲方法；",
    "- 默认输出一版完整可拍摄口播正文（可直接录），不要写成摘要，不要停在半句话；",
    "- 像真人面对镜头说话，短句为主，禁止营销黑话。",
...
const GENERAL_STORY_SKILL: AimWorkbenchSkill = {
  ...
  prompt: [
    ...
    "- 主线：真实经历 / 转折 / 现场细节 → 个人判断 → 让观众觉得「这个人值得信」；",
    "- 不强行推产品、不强行成交；信任来自真实细节而不是口号；",
