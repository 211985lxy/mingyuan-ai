...
import {
  agentAllowsContentModeSelector,
  agentAllowsVideoCopyExtraction,
...
export function validateGenerateInput(parsed: {
  ...
  if (parsed.videoCopyExtractionId && !agentAllowsVideoCopyExtraction(parsed.agentId)) {
    return "当前专家未授权视频文案提取"
  }
