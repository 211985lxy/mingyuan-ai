...
节奏：开头3秒强钩子（真能对上正文，不标题党）→ 持续给出新信息点或断裂感，让用户有理由再看一会儿、甚至想倒回去重看 → 提供可收藏的价值点（清单/步骤/模板/对照表/易错点/判断公式，可自然说「这个先收藏，后面用得上」，但不空洞喊收藏）→ 结尾把最值得回看的一句话做句锚，或给「下次遇到X场景就用Y方法」的复看提示。评论/转发引导只在用户要互动时给。
...
禁忌：只讲优惠；只讲功能；所有人都适合；结尾路径太复杂；传播内容结尾硬塞广告。`,
  ...
  },
  {
    ...
    structureModules: ["可替换开头", "钩子类型标注"],
    promptBody: `只优化开头，不重写整篇。调用七大爆款开头：好奇/借势/痛点/极限/恐吓/反差/利益输送。输出可替换开头并说明钩子类型；数量只按用户指令，没说先问一句要几个。`,
    antiPatterns: ["顺手重写整篇"],
    priority: 30,
  },
  {
    ...
    structureModules: ["标题候选", "主推版本"],
    promptBody: `标题规则：利益前置、痛点直击、反差冲突、人群点名、结果承诺。标题数量只按用户指令并标注主推；没说数量时先问一句要几个。`,
    antiPatterns: ["空泛标题"],
    priority: 30,
  },
  {
