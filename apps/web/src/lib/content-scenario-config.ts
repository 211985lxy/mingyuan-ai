...
export const SCENARIO_CONFIGS: Record<ContentScenario, ScenarioConfig> = {
  ip_knowledge: {
    promptBlock: [
      ...
      "- 开头第一句必须是一个反常识的观点或令人意外的数据，打破听众惯性思维",
      "- 每个论点必须附带一个具体案例（客户经历、行业故事或历史事件），禁止纯讲道理",
      "- 全篇控制在300字以内，口语化表达，去掉书面语和长句",
  ...
  entity_local: {
    promptBlock: [
      ...
      "- 植入至少2个可感知的细节（口味、环境、价格、服务），让用户产生画面感",
  ...
  xhs_planting: {
    promptBlock: [
      ...
      "- 标题必须包含品类关键词+情绪词或数字，长度控制在20字以内",
      "- 正文开头用一个「真实使用场景」切入，让用户产生代入感",
      "- 至少对比2个同类产品，突出本品的核心优势，用具体参数或体感佐证",
      "- 使用表情符号分隔段落，保持视觉节奏感，但每段不超过3行",
      "- 结尾用开放式提问引导互动（你在用什么？评论区告诉我），提升互动率",
  ...
  kol_explore: {
    promptBlock: [
      ...
      "- 至少设置一个「反转时刻」（预期差、意外惊喜、隐藏菜单），制造内容记忆点",
