...
const WX = ['channels.weixin.qq.com', 'finder.video.qq.com']
...
function plat(u: string): CommentRadarPlatform | null { const l = u.toLowerCase(); if (DD.some(d => l.includes(d))) return 'douyin'; if (XD.some(d => l.includes(d))) return 'xiaohongshu'; if (WX.some(d => l.includes(d))) return 'wechat_channels'; return null }
...
function extract(platform: CommentRadarPlatform, url: string) {
  try {
    ...
    if (platform === 'wechat_channels') { let m = p.match(/\/feed\/([\w]+)/); if (m) return { t: 'video' as const, id: m[1] }; m = p.match(/\/pages\/profile\/([\w]+)/); if (m) return { t: 'account' as const, id: null as string | null }; m = p.match(/\/mfinder\/([\w]+)/); if (m) return { t: 'account' as const, id: null as string | null } }
  } catch {}
  return null
}
/**
 * @description 解析source
 * @param raw - 原始数据
 * @param vl - vl
