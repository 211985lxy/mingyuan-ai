...
import { FeishuKnowledgeSourceCards } from "@/components/aim/feishu-knowledge-source-cards"
...
function MessageContent({ message, showThinkingProcess }: {
  ...
  if (message.role === "assistant") {
    ...
    const showMethodNote = Boolean(display.methodNote)
      && showThinkingProcess
      && !message.traceId
    return <>{showMethodNote ? <details className="mb-3 rounded-md border border-border bg-muted/25 px-3 py-2.5 text-sm text-muted-foreground"><summary className="cursor-pointer select-none font-medium text-foreground/80">思考依据</summary><div className="mt-2 border-t border-border/60 pt-2"><MarkdownRenderer content={display.methodNote} /></div></details> : null}<MarkdownRenderer content={display.result} />{message.deliverables ? null : <FeishuKnowledgeSourceCards sources={message.feishuSources} />}</>
  }
