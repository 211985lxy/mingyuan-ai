      ...
      <div className="rounded-xl border border-primary/20 bg-card shadow-sm">
        ...
        <div className="flex items-center gap-2 border-b px-4 py-2.5">
          ...
          <span className="text-xs font-medium text-muted-foreground">
            问题 {questionNumber}{totalQuestions > 0 ? ` / ${totalQuestions}` : ""}
          </span>
          <div className="ml-auto flex gap-1">
            {Array.from({ length: Math.max(totalQuestions, questionNumber) }).map((_, i) => (
              <span
                key={i}
        ...
        <div className="flex flex-col gap-2 px-4 pb-3">
          {question.options.map((option) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                {option.sourceRefs.length > 0 && (
                  <span className="mt-1 flex flex-wrap gap-1">
                    {option.sourceRefs.map((ref, i) => (
          ...
          {!customMode ? (
            <button
              type="button"
              disabled={loading}
              className="group flex w-full items-center gap-3 rounded-lg border border-dashed border-border/60 px-3.5 py-2.5 text-left transition-all hover:border-primary/40 hover:bg-primary/5 disabled:opacity-50"
              onClick={() => setCustomMode(true)}
            >
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-dashed border-muted-foreground/30 text-[11px] font-semibold text-muted-foreground transition-colors group-hover:border-primary group-hover:text-primary">
                D
              </span>
