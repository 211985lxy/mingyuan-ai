...
import { ArrowLeft, ArrowUp, Plus, Sparkles, Square } from "lucide-react"
...
import { isValidAimAgent, getAimAgent } from "@/lib/aim-ui-config"
...
function LiteChatInner() {
  const searchParams = useSearchParams()
  const agentParam = searchParams.get("agent")
  // 非法/隐藏专家一律回落通用大脑，避免把任意 id 透传给后端
  const agent = agentParam && isValidAimAgent(agentParam) ? getAimAgent(agentParam) : null
  const agentId = agent?.id
  const { messages, busy, send, stop, reset } = useLiteChat(agentId)
      ...
      <div className="flex shrink-0 items-center gap-2 border-b bg-muted/30 px-4 py-2">
        ...
        {agent ? (
          <span className="flex min-w-0 items-center gap-2">
            ...
            <span className="hidden truncate text-xs text-muted-foreground sm:inline">{agent.description}</span>
          </span>
        ) : (
          <span className="flex min-w-0 items-center gap-2">
            <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary">
            ...
            <span className="truncate text-sm font-medium">明远 AIM · 通用大脑</span>
            <span className="hidden truncate text-xs text-muted-foreground sm:inline">通用问答、写作与一次性任务</span>
          </span>
        )}
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {messages.length === 0 ? (
          <div className="mx-auto flex h-full w-full max-w-3xl flex-col items-center justify-center px-4 py-10 text-center">
            <p className="text-xs font-medium text-primary">{agent ? agent.title : "明远 AIM"}</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight">今天想聊点什么？</h1>
            <p className="mt-1.5 text-sm text-muted-foreground">
              {agent ? agent.description : "直接输入问题或需求，AIM 帮你写文案、理思路、做复盘。"}
            </p>
