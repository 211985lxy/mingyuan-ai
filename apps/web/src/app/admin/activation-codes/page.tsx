    ...
    <AdminPageShell
      ...
      actions={
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            <GenForm onSuccess={onGen} />
          </DialogContent>
        </Dialog>
      }
      stats={
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard2 title="总数" value={stats?.total} />
          <StatCard2 title="未使用" value={stats?.unused} />
          <StatCard2 title="已使用" value={stats?.used} />
          <StatCard2
            title="使用率"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/50">
                <th className="p-3 text-left font-medium">激活码</th>
                <th className="p-3 text-left font-medium">状态</th>
                <th className="p-3 text-left font-medium">有效期</th>
                <th className="p-3 text-left font-medium">批次</th>
                <th className="p-3 text-left font-medium">使用者</th>
                <th className="p-3 text-left font-medium">使用时间</th>
            ...
            <tbody>
              {codes.map((c) => (
                <tr key={c.id} className="border-b hover:bg-muted/30">
                  ...
                  <td className="p-3">
                    <Badge
                      variant={c.status === "used" ? "default" : "secondary"}
                    >
...
function GenForm({ onSuccess }: { onSuccess: () => void }) {
  const [qty, setQty] = React.useState("50");
  const [dur, setDur] = React.useState("14");
  ...
  async function submit(e: React.FormEvent) {
    ...
    setErr(null);
    try {
      const r = await generateActivationCodes(n, d, note || undefined);
      setResult({ count: r.data.count, durationDays: r.data.durationDays });
  ...
  if (result)
    return (
      <div className="text-center py-6">
        ...
        <p className="text-lg font-medium">已生成{result.count}个</p>
      </div>
    );
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>批量生成激活码</DialogDescription>
      </DialogHeader>
      <form onSubmit={submit} className="space-y-4">
        ...
        <Button type="submit" disabled={loading} className="w-full">
          {loading && <Loader2 className="h-4 w-4 animate-spin mr-2" />}生成
        </Button>
