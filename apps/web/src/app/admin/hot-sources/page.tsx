...
const EMPTY: AdminHotSourceInput = {
  email: "",
  sourceName: "",
  sourceUrl: "",
  sourceType: "static",
...
export default function AdminHotSourcesPage() {
  ...
  const [selected, setSelected] = React.useState<AdminHotSourceInput>(EMPTY);
  ...
  const fetchSources = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await getAdminHotSources();
      setSources(r.data);
      if (r.data[0]) setSelected(toI(r.data[0]));
    } catch (e) {
  ...
  return (
    <AdminPageShell
      title="热点信源"
      subtitle="按账号绑定热点精选使用的行业信源。"
      loading={loading}
      error={error}
      onRetry={fetchSources}
      skeletonRows={3}
      empty={!loading && !error && sources.length === 0}
      emptyMessage="还没有热点信源。"
      actions={
            ...
            <form onSubmit={save} className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setSelected((p) => ({ ...p, enabled: !p.enabled }))
                  }
                >
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setSelected((p) => ({ ...p, email: e.target.value }))
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setSelected((p) => ({ ...p, sourceName: e.target.value }))
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setSelected((p) => ({ ...p, sourceUrl: e.target.value }))
                  }
                />
              </div>
              <div>
                ...
                <Textarea
                  ...
                  rows={3}
                  onChange={(e) =>
                    setSelected((p) => ({ ...p, note: e.target.value }))
                  }
                />
              </div>
...
function toI(s: AdminHotSourceItem): AdminHotSourceInput {
  return {
    email: s.email,
