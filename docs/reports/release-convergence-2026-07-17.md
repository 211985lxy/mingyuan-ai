...
仓库当前没有 Git remote。提交仍只在本机；磁盘故障会造成不可恢复损失。

- 缓解：配置私有远程仓库，推送 `main`、唯一候选分支和必要标签，并从远程重新拉取一次验证。
- 责任人：仓库所有者。
...
当前可对团队表述为：“候选代码已完成本地工程验收，尚未获准生产发布。剩余工作不是继续写功能，而是远程备份、破坏性迁移演练、真实模型和飞书链路验收。”
