...
根因不是闸门太严，是教训写在知识前面，模型把「禁止编造未给出的数字」听成「稿子里不能出现数字」，知识库账单 1800→1100 用不上，改去交分析。

改动：知识事实在前；抽出已给出数字并标明「写进口播不算编造」；评测与生产共用 `mergeLearningsIntoKnowledge`；闸门重试明确要求用知识库数字写稿、不能改交分析。
...
| 连续 3 次真实模型 daily 绿色 | 未达成。公开仓后再跑一次 `34742150831`：合同 86.7%，rubric 88.5%。`cp_info_insufficient_20` 已过；仍挂 `cp_learnings_hallucination_26` 两次、`cp_imitate_07`/`pq_ground_16` 各一次 |
| model-swap | 未跑 |
