...
## 附录 L：蝉镜侧收尾与两次生产部署（2026-09-13，承接附录 J）

附录 K 记录的是 HeyGen 线。本附录补录同期货真价实的蝉镜侧收尾，避免交付记录断档：

| PR | 内容 | merge commit |
| --- | --- | --- |
| #66 | **公共数字人可直接出片**（无需克隆与授权文案）：列表路由（服务端代理）+ 对话框来源切换；顺带修「换公共形象被判同一单」的幂等缺陷（该缺陷由新增 e2e 发现，单测不可见） | `499b9121` |
| #70 | **access_token 有效期修复**：实测供应商返回的是绝对时间戳，原实现按相对时长缓存 ≈ 56 年，令牌永不主动刷新（仅 10400 重试兜底） | `670ae808` |
| #74 | 仓库转 public 后的凭证清扫（详见 K.3；轮换仍待做） | `b2e42bfe` |
| #62 | CI 额度治理：并发取消 + main 去重；PR 全量 / main 构建门禁两条路径均已真实运行验证 | `a6a47968` |

### L.1 生产部署记录（两次，均以 healthz releaseSha 回读核验）

| 次序 | releaseSha | 内容 |
| --- | --- | --- |
| 1 | `499b9121` | 公共数字人 + 蝉镜凭证首次写入生产并验证（鉴权 code=0） |
| 2 | `670ae808` | token 修复上线；12 项集成探针 healthy |

之后生产由 HeyGen 线继续推进至 `f791d610`（#75）。

### L.2 上线后仍开放的项

- 克隆路径等控制台授权文案（公共数字人路径不受影响）
- 轮换 4 个已暴露凭证（见 K.3 与附录 H）
