...
| WP-7 资产沉淀 | 代码完成（生产迁移待发布） | `AssetCandidate`、会议审核闸门、候选审批与正式知识晋升 | 未经双重人工批准不能进入正式知识，项目隔离测试通过 |
| WP-8 无人值守 | 代码完成（影子灰度通过，live 未启用） | Cron 调度、原子幂等、租约、失败分类、人工接管、监督通知、灰度开关 | 真实 Base 影子运行成功；自动外部副作用仍关闭 |
...
上线前阻塞：生产镜像尚未安装并配置 `lark-cli`，本地真实联调通过不等于已上线。
