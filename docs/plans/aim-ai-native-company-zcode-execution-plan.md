...
> 状态：代码已部署，生产 Loop 关闭，扩大灰度中
> 创建日期：2026-07-16  
> 业务负责人：李相宇  
> 架构、审查与验收：Codex（GPT-5.6 Sol）  
> 实现执行：Z-Code（前期工作包）+ Codex（Loop 收敛、审查、灰度与发布）
> 当前生产基线：`main@a67f2bac7952ea52ae12d956e8102313ffd938a3`
...
| WP-2 经营事项状态机 | 已完成 | 纯领域类型、解析、合法转换、字段 patch | 30 项单测、类型和架构检查通过 |
...
| WP-6 客户会后工作流 | 已完成（影子灰度通过） | 真实会议原文经 Harness 抽取、确定性验证、AimGeneration 落盘并进入人工审核 | 中汝达真实样本 13/13 检查通过，Trace/Snapshot/Generation 可追溯 |
| WP-7 资产沉淀 | 已部署（功能闸门关闭） | `AssetCandidate`、会议审核闸门、候选审批与正式知识晋升 | 生产表与 Schema 合同通过；未经双重人工批准不能进入正式知识 |
| WP-8 无人值守 | 已部署（影子通过，live 未启用） | Cron 调度、原子幂等、租约、失败分类、人工接管、监督通知、灰度开关 | 真实 Base 影子运行成功；生产未配置飞书和 Loop 开关，自动外部副作用关闭 |
...
目标 Base 已确认：`明远-选题脚本与数据分析系统`。

- 经营事项表：`AIM经营事项`
- 表 ID：`tbluPSktbNDNw0T7`
- 默认视图 ID：`vewAUlJ8VC`
- WP-5 初次联调时回读了 11 个基础字段；截至 2026-07-19，真实表已扩展为 31 个字段，当前字段结构已重新只读回查，生产启用状态仍以灰度开关和发布门禁为准。

联调结果：

1. 已验证机器人身份的字段、记录读写权限。
2. 已通过真实 Base token 和表 ID 调用经营事项执行入口。
3. 验收记录 `recvpwQ04r3TvR` 已完成主链、幂等和终态拒绝验证。
4. 真实联调修复了 bot 身份传递、CLI 矩阵响应解析和 URL 字段写入契约。

生产状态：Loop 代码已经部署，但生产环境未配置飞书凭据、`lark-cli`、试点项目和监督群，且总开关保持默认关闭。只有完成 10 场真实销售会议和连续 5 个工作日观测后，才允许进入生产 live。
...
- **抽取入口**：复用现有 AIM Harness 和 `business_diagnosis` 模型路由，不新增 Agent 运行时。
- **状态控制**：飞书 `AIM经营事项` 继续使用唯一五态状态机；Loop 契约不建立第二套业务状态。
- **审批边界**：模型结果通过确定性验证后仍只进入 `待人工审核`，客户沟通、发布、报价和知识晋升均不得自动执行。
- **产物正本**：完整洞察、验证证据、模型与成本元数据落在 AimGeneration/Trace/Snapshot；Base 只承担执行控制和摘要投影。
- **真实验收**：2026-07-19 已用中汝达真实逐字稿完成单样本影子闭环，13/13 检查通过；10 场会议的业务通过率门槛仍属于扩大灰度阶段，不等同于已完成生产放量。

## 16. 生产发布收口（2026-07-19）

- 已发布 `main@a67f2bac7952ea52ae12d956e8102313ffd938a3`，生产健康接口连续三次返回该精确 SHA。
- 发布门禁通过：Prisma Client 生成、TypeScript、125 项 Harness 测试、架构护栏、构建、独立制品依赖、生产 Schema 补丁与 11 表 Schema 合同。
- 生产数据库已补齐 `AimExecutionTrace` 的 token/成本遥测字段，并纳入后续发布硬门禁。
- 生产 APIMart 模型已从会产生空响应的 `gpt-5` 改为影子灰度验证通过的 `gpt-5.2-chat-latest`。
- 无副作用冒烟通过：健康接口 200、登录页 200、无授权 Cron 401；部署后日志未再出现 `P2022` 或 Prisma 验证错误。
- 回滚制品保留上一生产版本 `5268e5f411ce701bb6ae6b26a5c22b40ab017281`；Loop live 与飞书主动通知仍保持关闭。
