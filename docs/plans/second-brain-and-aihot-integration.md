...
| **attraction** (吸引力) | **7 分** | 前 3 秒钩子（Hook）强度、价值留人（是否值得收藏与复访，即可持续停留能力）、悬念设置；**不以完播率作为判据**（抖音等平台已不考察完播率，长视频完播率天然偏低但收藏/复访/铁粉互动好依然高吸引力） | 调用 `HOOK_REWRITE_PROMPT` 靶向开头 3 秒重构，保留正文 |
